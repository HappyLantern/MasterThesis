...
for filename in os.listdir("partial sthlmgrey"):
    with rasterio.open("partial sthlmgrey/"+filename) as tile: #  'w', driver="GTiff"
        #array = tile.read(1)
        ...
        with collection("shapefiles etc/sthlmpoly.shp", "r") as input: # OBS hårdkodat för sthlmgrey
            for point in input:
                ...
                och 6583620 - 6580192 = 3428 i y-led på 7380, ger 1.879, 2,153
                ...
                for coords in p['coordinates'][0]: # 326525.8594834,6570069.2548295 är start
                    coords[0] -= 326525.8594834    # 343906.6801505,6583620.1083060 är slut
                    coords[1] -= 6583620.1083060   # ger 17380.8207 i x-led, 13550.8535 i y-led fördelat på 33469,27868
                    coords[0] *= 2          # dvs multiplicerar vi t ex resten i x-led med 33469/17380.8 = 1.92563
                    coords[1] *= -2            # och i y-led blir det 27868/13550.8535 = 2.05655
                #print(coords)
