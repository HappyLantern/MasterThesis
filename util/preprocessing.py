...
def main(argv):
   ...
   with open(inputfile, 'r') as cfg:
      ...
      orig_data_dir = content[0][content[0].find(':') + 2 : -1]
      ...
      data_classes = [x[0 : -1] for x in data_classes]
      nbr_images   = {}

      for data_class in data_classes:

         curr_orig_dir  = os.path.join(orig_data_dir, data_class)     
         ...
         nbr_images[data_class] = len(os.listdir(os.path.join(orig_data_dir, data_class)))
      ...
      for data_class in data_classes:
         ...
         fnames = [filename + '.{}.txt'.format(i) for i in range(train_range)]
         for fname in fnames:
            src = os.path.join(orig_data_dirs[data_class], fname)
            dst = os.path.join(train_dirs[data_class], fname)
            shutil.copyfile(src, dst)
         
         fnames = [filename + '.{}.txt'.format(i) for i in range(train_range, val_range)]
         for fname in fnames:
            src = os.path.join(orig_data_dirs[data_class], fname)
            dst = os.path.join(val_dirs[data_class], fname)
            shutil.copyfile(src, dst)
         
         fnames = [filename + '.{}.txt'.format(i) for i in range(val_range, test_range)]
         for fname in fnames:
            src = os.path.join(orig_data_dirs[data_class], fname)
